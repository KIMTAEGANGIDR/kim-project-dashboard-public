...
async function refreshHealth() {
  ...
  if (!strip) return;
  try {
    const res = await fetch("./automation_health.json", { cache: "no-store" });
    if (!res.ok) throw new Error(String(res.status));
    const health = await res.json();
    const counts = health.counts || {};
    setText("healthOk", number.format(counts["정상"] ?? 0));
    setText("healthWarn", number.format(counts["주의"] ?? 0));
    setText("healthStopped", number.format(counts["정지"] ?? 0));
    const daemons = Object.values(health.launchd || {});
    const alive = daemons.filter((v) => v === "가동" || v === "등록").length;
    setText("healthDaemons", `${alive}/${daemons.length}`);
    setText("healthDrift", number.format((health.drift || []).length));
    setText("healthUpdated", `점검 ${fmtDate(health.generated_at)}`);
    const items = [
      ...(health.issues || []).map((text) => ({ kind: "주의", text })),
      ...(health.drift || []).map((text) => ({ kind: "장부", text }))
    ];
    document.getElementById("healthIssues").innerHTML = items.length
      ? items.map((item) => `<li class="${item.kind === "주의" ? "issue-warn" : "issue-drift"}"><span>${escapeHtml(item.kind)}</span>${escapeHtml(item.text)}</li>`).join("")
      : `<li class="issue-ok"><span>정상</span>모든 자동 작업이 정상입니다.</li>`;
    strip.hidden = false;
  } catch (err) {
    strip.hidden = true; // 공개 배포본 등 데이터가 없는 환경에서는 섹션을 숨긴다
  }
}
...
async function boot() {
  ...
  refreshHealth();
  window.setInterval(refreshHealth, 60_000);
